...
#define PKT_FLAGS                            \
    ...
    pf(TUNSRC_RND)    /* ip tunnel src random */        \
    pf(TUNDST_RND)    /* ip tunnel dst random */        \
    pf(TUN_IPV6)                                    \
    pf(INNER_MACSRC_RND)                            \
    pf(INNER_MACDST_RND)                            \
    pf(TUNMETA_RND)                                    \
...
struct pktgen_dev {
    ...
    char tun_src_min[IP_NAME_SZ];
    char tun_src_max[IP_NAME_SZ];
    char tun_dst_min[IP_NAME_SZ];
    char tun_dst_max[IP_NAME_SZ];

    struct in6_addr tun6_saddr;
    struct in6_addr tun6_daddr;
    struct in6_addr cur_tun6_daddr;
    struct in6_addr cur_tun6_saddr;

    __be32 tun_saddr_min;
    __be32 tun_saddr_max;
    __be32 tun_daddr_min;
    __be32 tun_daddr_max;

    __be32 cur_tun_vni;
    __be32 tun_udp_dst;

    uint32_t tun_vni_min;
    uint32_t tun_vni_max;
    unsigned char inner_dst_mac[ETH_ALEN];
    unsigned char inner_src_mac[ETH_ALEN];
    __u32 inner_src_mac_count;
    __u32 inner_dst_mac_count;
    __u32 cur_inner_dst_mac_offset;
    __u32 cur_inner_src_mac_offset;
    ...
    __be32 cur_saddr;
    __be32 cur_daddr;
    __be32 cur_tun_saddr;
    __be32 cur_tun_daddr;
    __u16 ip_id;
    __u16 tun_ip_id;
    __u16 cur_udp_dst;
    __u16 cur_udp_src;
                   ...
                   loff_t * offset)
{
    ...
    if (!strcmp(name, "tun_dst_min") || !strcmp(name, "tun_dst")) {
        len = strn_len(&user_buffer[i], sizeof(pkt_dev->tun_dst_min) - 1);
        if (len < 0)
            return len;
        ...
        if (copy_from_user(buf, &user_buffer[i], len))
            return -EFAULT;
        buf[len] = 0;
        if (strcmp(buf, pkt_dev->tun_dst_min) != 0) {
            memset(pkt_dev->tun_dst_min, 0, sizeof(pkt_dev->tun_dst_min));
            strcpy(pkt_dev->tun_dst_min, buf);
            pkt_dev->tun_daddr_min = in_aton(pkt_dev->tun_dst_min);
            pkt_dev->cur_tun_daddr = pkt_dev->tun_daddr_min;
        }
        if (debug)
            pr_debug("tun_dst_min set to: %s\n", pkt_dev->tun_dst_min);
        i += len;
        sprintf(pg_result, "OK: tun_dst_min=%s", pkt_dev->tun_dst_min);
        return count;
    }
    if (!strcmp(name, "tun_dst_max")) {
        len = strn_len(&user_buffer[i], sizeof(pkt_dev->dst_max) - 1);
        if (len < 0)
            return len;
        ...
        buf[len] = 0;
        if (strcmp(buf, pkt_dev->dst_max) != 0) {
            memset(pkt_dev->tun_dst_max, 0, sizeof(pkt_dev->tun_dst_max));
            strcpy(pkt_dev->tun_dst_max, buf);
            pkt_dev->tun_daddr_max = in_aton(pkt_dev->tun_dst_max);
            pkt_dev->cur_tun_daddr = pkt_dev->tun_daddr_max;
        }
        if (debug)
            pr_debug("tun_dst_max set to: %s\n", pkt_dev->tun_dst_max);
        i += len;
        sprintf(pg_result, "OK: tun_dst_max=%s", pkt_dev->tun_dst_max);
        return count;
    }
    if (!strcmp(name, "tun_src_min")) {
        len = strn_len(&user_buffer[i], sizeof(pkt_dev->tun_src_min) - 1);
        if (len < 0)
            return len;
        ...
        if (copy_from_user(buf, &user_buffer[i], len))
            return -EFAULT;
        buf[len] = 0;
        if (strcmp(buf, pkt_dev->tun_src_min) != 0) {
            memset(pkt_dev->tun_src_min, 0, sizeof(pkt_dev->src_min));
            strcpy(pkt_dev->tun_src_min, buf);
            pkt_dev->tun_saddr_min = in_aton(pkt_dev->tun_src_min);
            pkt_dev->cur_tun_saddr = pkt_dev->tun_saddr_min;
        }
        if (debug)
            pr_debug("tun_src_min set to: %s\n", pkt_dev->tun_src_min);
        i += len;
        sprintf(pg_result, "OK: tun_src_min=%s", pkt_dev->tun_src_min);
        return count;
    }
    if (!strcmp(name, "tun_src_max")) {
        len = strn_len(&user_buffer[i], sizeof(pkt_dev->tun_src_max) - 1);
        if (len < 0)
            return len;
        ...
        if (copy_from_user(buf, &user_buffer[i], len))
            return -EFAULT;
        buf[len] = 0;
        if (strcmp(buf, pkt_dev->tun_src_max) != 0) {
            memset(pkt_dev->tun_src_max, 0, sizeof(pkt_dev->tun_src_max));
            strcpy(pkt_dev->tun_src_max, buf);
            pkt_dev->tun_saddr_max = in_aton(pkt_dev->tun_src_max);
            pkt_dev->cur_tun_saddr = pkt_dev->tun_saddr_max;
        }
        if (debug)
            pr_debug("tun_src_max set to: %s\n", pkt_dev->tun_src_max);
        i += len;
    ...
    if (!strcmp(name, "inner_smac_num")) {
        ...
        i += len;
        if (pkt_dev->inner_src_mac_count != value) {
    ...
    if (!strcmp(name, "inner_dmac_num")) {
        ...
        i += len;
        if (pkt_dev->inner_dst_mac_count != value) {
...
static void pktgen_setup_inject(struct pktgen_dev *pkt_dev)
{
        ...
        if (pkt_dev->tun_vni_min != 0 && pkt_dev->tun_udp_dst != 0) {
            if (strlen(pkt_dev->tun_src_min) == 0) {
                ...
                if (in_dev) {
                    ...
                    if (ifa) {
                        pkt_dev->tun_saddr_min = ifa->ifa_address;
                        pkt_dev->tun_saddr_max = pkt_dev->tun_saddr_min;
                    }
                }
                rcu_read_unlock();
            } else {
    ...
    pkt_dev->cur_saddr = pkt_dev->saddr_min;
    pkt_dev->cur_tun_saddr = pkt_dev->tun_saddr_min;
    pkt_dev->cur_tun_vni = pkt_dev->tun_vni_min;
    pkt_dev->cur_daddr = pkt_dev->daddr_min;
...
static void mod_cur_headers(struct pktgen_dev *pkt_dev)
{
    ...
    if ((!(pkt_dev->flags & F_TUN_IPV6)) && pkt_dev->tun_vni_min) {
        ...
        if (imn < imx) {
            ...
            pkt_dev->cur_tun_saddr = htonl(t);
        }

        if (pkt_dev->cflows && f_seen(pkt_dev, flow)) {
            pkt_dev->cur_tun_daddr = pkt_dev->flows[flow].cur_daddr;
        } else {
            imn = ntohl(pkt_dev->tun_daddr_min);
            imx = ntohl(pkt_dev->tun_daddr_max);
            if (imn < imx) {
                __u32 t;
                __be32 s;
                if (pkt_dev->flags & F_TUNDST_RND) {
                    do {
                        ...
                        ipv4_is_zeronet(s) ||
                        ipv4_is_local_multicast(s));
                    pkt_dev->cur_tun_daddr = s;
                } else {
                    t = ntohl(pkt_dev->cur_tun_daddr);
                    t++;
                    if (t > imx) {
                        t = imn;
                    }
                    pkt_dev->cur_tun_daddr = htonl(t);
                }
            }
            if (pkt_dev->cflows) {
                pkt_dev->flows[flow].flags |= F_INIT;
...
static void fill_skb_mpls(struct sk_buff *skb, struct pktgen_dev *pkt_dev)
{
    __be32 *mpls;
...
static void fill_skb_vlan(struct sk_buff *skb, struct pktgen_dev *pkt_dev)
{
...
static void fill_udp_layer(struct sk_buff *skb,
...
static struct sk_buff *fill_tun_packet_ipv6(struct net_device *odev,
                    struct pktgen_dev *pkt_dev)
{
    struct sk_buff *skb = NULL;
    __u8 *eth;
...
static struct sk_buff *fill_tun_packet(struct net_device *odev,
                   struct pktgen_dev *pkt_dev)
{
    if (pkt_dev->flags & F_TUN_IPV6)
        return fill_tun_packet_ipv6(odev, pkt_dev);
    else
...
static void pktgen_run(struct pktgen_thread *t)
{
    ...
    list_for_each_entry_rcu(pkt_dev, &t->if_list, list) {
        ...
        if (pkt_dev->odev) {
            ...
            pkt_dev->mburst_tx = ktime_add_ms(pkt_dev->next_tx, 300);
            set_pkt_overhead(pkt_dev);
...
static void pktgen_xmit(struct pktgen_dev *pkt_dev)
{
    ...
    if (!pkt_dev->skb || (pkt_dev->last_ok &&
        ...
        if (pkt_dev->tun_vni_min || pkt_dev->tun_udp_dst) {
            pkt_dev->skb = fill_tun_packet(odev, pkt_dev);
        } else {
    ...
    if (pkt_dev->pause_time) {
        pkt_dev->next_tx = ktime_get();
        next_stop = ktime_add_ms(pkt_dev->next_tx, pkt_dev->pause_time);
        if (ktime_compare(pkt_dev->mburst_tx, pkt_dev->next_tx) > 0 &&
